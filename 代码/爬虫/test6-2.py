...
def build_form(press_name):
    ...
    # 定位 input 标签
    input_tag_name = ''
    conditions = soup.select('.box.box2,clearfix>.detail_condition > label')
    print('共找到%d 项基本条件,正在寻找 input 标签' %
len(conditions))
...
def get_info(entry_url):
    ...
    print('共 %d 页待抓取， 这里只测试采集 2 页' % page_num)
    page_num = 2
    ...
    for i in range(1, page_num + 1):
        ...
        # 获取评论数量
        tmp_books_comment = soup.select('ul.bigimg>li[ddt-pit]>p.search_star_line')
        # print(tmp_books_comment)
        for book in tmp_books_comment:
            # print(book.text)
            if book.text != '':  # is not null
                books_comment.append(book.text)
                # print(type(book.text))
            else:
                books_comment.append('0 条评论')
                # print('0')
        # 获取出版日期
...
if __name__ == '__main__':
    # 出版社名列表所在文件路径
    press_txt_path = r’_______________'
    saved_file_dir = r'______________________________________________________'
    # 启动
